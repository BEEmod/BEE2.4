...
import attrs
from srctools import Vec, FrozenVec, Angle, Matrix, FrozenMatrix
...
from precomp.texturing import TileSize, Portalable, MaterialConf
from . import (
    instanceLocs,
    texturing,
    options,
...
TILETYPE_TO_SKIN = {typ: ind for ind, typ in enumerate(SKIN_TO_TILETYPE)}
...
class TileDef:
    ...
    override: tuple[texturing.QuarterRot, template_brush.ScalingTemplate] | None
    ...
    def export(self, vmf: VMF) -> None:
        ...
        if self.use_bullseye():
            # Add the bullseye overlay.
            mat_conf = texturing.OVERLAYS.get(front_pos, 'bullseye')
            orient = mat_conf.rotation.mat_x @ Matrix.from_angle(self.normal.to_angle())
...
def make_tile(
    ...
    back_surf: MaterialConf=MaterialConf(consts.Tools.NODRAW.value),
    *,
    recess_dist: int=0,
    thickness: int=4,
    width: float=16,
