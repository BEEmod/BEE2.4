...
def flag_and(inst: Entity, flag: Property):
    ...
    for i, sub_flag in enumerate(flag):
        if not check_flag(sub_flag, inst, can_skip=i == 0):
            return False
    return True
...
def flag_not(inst: Entity, flag: Property):
    ...
    try:
        ...
        return False
    return not check_flag(subflag, inst)
...
@make_flag('NOR')
def flag_nor(inst: Entity, flag: Property) -> bool:
...
@make_flag('NAND')
def flag_nand(inst: Entity, flag: Property) -> bool:
